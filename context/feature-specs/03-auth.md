...
Use `proxy.ts` at the project root, not `middleware.ts` if the Next Js version is greater than to 15.0, otherwise use `middleware.ts`. The code itself remains the same; only the filename changes.
